...
def respuesta(datos, estado=status.HTTP_200_OK):
    # if str(type(datos)).__contains__("ReturnList"):
    #     return JsonResponse({"largo":len(datos),"datos": datos}, status=estado)
    # return JsonResponse({"tipo":str(type(datos)),"datos": datos}, status=estado)
